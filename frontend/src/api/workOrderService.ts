...
export type WoStatus = 'DRAFT' | 'PENDING' | 'APPROVED' | 'MATERIAL_ISSUED' | 'PACKING_STARTED' | 'PACKING_IN_PROGRESS' | 'PACKING_COMPLETED' | 'LABEL_APPLICATION_ASSIGNED' | 'LABEL_APPLICATION_IN_PROGRESS' | 'LABELS_APPLIED' | 'QC_PENDING' | 'QC_IN_PROGRESS' | 'QC_PASSED' | 'COMPLETED' | 'CANCELLED';
...
export const workOrderService = {
  ...
  async issueMaterials(id: string, payload: any): Promise<{ issue: MaterialIssueRecord; updatedWO: WorkOrder }> {
    const res = await apiClient.post<{ success: boolean; data: { issue: MaterialIssueRecord; updatedWO: WorkOrder } }>(`/work-orders/${id}/issue-materials`, { payload });
    return res.data.data;
  },
