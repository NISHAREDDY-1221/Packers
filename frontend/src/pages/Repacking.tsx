...
export const Repacking: React.FC = () => {
  const { repackings, recipes, workOrders, qualityChecks, addRepacking } =
    useApp();

  const failedQCs = qualityChecks.filter((qc) =>
    ["Reject", "Rework", "Discard"].includes(qc.result),
  );
  ...
  const pendingRepackQCs = failedQCs.filter((qc) => {
    ...
    const batchNo =
      qc.batchNo || wo.batchNumber || `BATCH-2026-${wo.woNo.split("-").pop()}`;
  ...
  const [selectedRecord, setSelectedRecord] = useState<RepackingRecord | null>(
    null,
  );
  ...
  const [formRepackType, setFormRepackType] = useState(
    "Damaged Pack → New Pack",
  );
          ...
          <div className="space-y-3">
            {pendingRepackQCs.map((qc) => {
              ...
              return (
                <div
                  key={qc.id}
                  className="p-4 border border-slate-200 hover:border-orange-400 rounded-xl bg-slate-50 transition-all space-y-3 text-xs"
                >
                  <div className="flex justify-between items-start">
                    ...
                    <span
                      className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase tracking-wide ${qc.result === "Rework" ? "bg-orange-100 text-orange-800" : "bg-rose-100 text-rose-800"}`}
                    >
                      {qc.result}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[11px]">
                    ...
                    <div>
                      ...
                      <span className="font-medium text-slate-700">
                        {qc.checkedQty || wo.actualProduced} units
                      </span>
                    </div>
                    <div className="col-span-2">
                      ...
                      <span
                        ...
                        title={qc.failureReason || qc.remarks}
                      >
                        {qc.failureReason || qc.remarks || "—"}
                      </span>
                    </div>
                    <div className="col-span-2">
                      <span className="text-slate-400 block text-[9px] uppercase font-bold">
                      ...
                      <span className="font-medium text-slate-700">
                        {formatDateTime(qc.completionTime || qc.date)}
                      </span>
                    </div>
                  </div>
                  ...
                  <button
                    onClick={() => {
                      handleSelectFailedBatch(batchNo);
                      setIsFormOpen(true);
                    }}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormRecoveredQty(Number(e.target.value))
                    }
                  />
                </div>
                <div>
