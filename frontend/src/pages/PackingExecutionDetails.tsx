...
export const PackingExecutionDetails: React.FC = () => {
    ...
    if (id) {
      workOrderService.getWorkOrders().then(res => {
        ...
        const mapped = (res.data || []).map((wo: any) => ({
          ...wo,
          status: formatStatus(wo.status),
          supervisor: getSupervisorName(wo.supervisor)
        }));
        const wo = mapped.find((w: any) => w.id === id);
        if (wo) {
          setActiveWO(wo);
        }
      });
  ...
  const fetchWorkOrders = useCallback(() => {
    workOrderService.getWorkOrders({ limit: 500 }).then(res => {
      const formatStatus = (s: string) => {
        ...
        if (s === 'PACKING_STARTED') return 'Packing Started';
        if (s === 'QC_PENDING') return 'QC Pending';
        if (s === 'QC_PASSED') return 'QC Passed';
  ...
  useEffect(() => {
    if (selectedWO && selectedWO.startedAt) {
      const start = new Date(selectedWO.startedAt).getTime();
      const end = selectedWO.completedAt ? new Date(selectedWO.completedAt).getTime() : new Date().getTime();
  ...
  const handleCompletePacking = async () => {
    if (!selectedWO || !activeWO) return;
    try {
      setActionLoading(true);
      // @ts-ignore
      await workOrderService.completePacking(activeWO.id, {
          ...
          {selectedWO && (
            <div className="space-y-6">
              ...
              <div className="flex items-center mb-4">
                ...
                >
                  <ArrowLeft size={16} /> Back to Packing Execution
                </button>
              </div>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-100 dark:border-gray-750 pb-3">
                  <div>
                    ...
                    <h3 className="font-bold text-lg text-slate-850 dark:text-gray-150">
                      {(selectedWO.product?.name || "Unknown Product")}
                    </h3>
                  </div>
                ...
                <div className="flex flex-wrap sm:flex-nowrap items-center justify-between gap-2 text-xs">
                  {[
                    ...
                    <React.Fragment key={stage}>
                      ...
                      >
                        {isCompleted && <Check size={16} className="inline-block mr-1" />}
                        {isActive && <Circle size={10} className="inline-block mr-1 fill-current" />}
                        {stage}
                      </div>
                      {idx < arr.length - 1 && (
                        <div className="flex items-center text-slate-300 dark:text-gray-600 px-2">
                      ...
                      <div className="bg-slate-50 dark:bg-gray-700 p-5 grid grid-cols-2 gap-4 text-center">
                        <div>
                          ...
                          <span className="font-bold text-slate-800 dark:text-gray-100 mt-1 block">
                            {progressMetrics.packed > 0 ? ((progressMetrics.packed / (progressMetrics.packed + progressMetrics.rejected)) * 100).toFixed(1) + "%" : "-"}
                          </span>
                        </div>
                        <div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="border border-slate-200 dark:border-gray-700 rounded-xl p-4 bg-slate-50/50 dark:bg-gray-750 space-y-3">
                      <div className="flex justify-between">
                        ...
                        <span className="font-bold text-slate-800 dark:text-gray-200">
                          {(activeRecipe?.name || activeRecipe?.packingName || selectedWO.recipeId)}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="font-semibold text-slate-505">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl max-w-md w-full shadow-2xl overflow-hidden border border-slate-200 dark:border-gray-700">
            ...
            <div className="p-5 space-y-4 text-sm">
              <p className="text-slate-600 dark:text-gray-300">
                ...
                <span className="font-bold">{selectedWO.woNumber}</span>. Please
