...
export const Approvals: React.FC = () => {
  ...
  const approvedTodayCount = approvals.filter(
    (a) => a.status === "APPROVED",
  ).length;
  const rejectedTodayCount = approvals.filter(
    (a) => a.status === "REJECTED",
  ).length;
  ...
  const getPriorityBadge = (priority: Priority) => {
    ...
    return (
      <span
        className={`px-2.5 py-1 text-[10px] font-bold rounded-full ${styles[priority]}`}
      >
        {priority}
      </span>
    );
  ...
  const getStatusBadge = (status: ApprovalStatus) => {
    ...
    return (
      <span
        className={`px-2.5 py-1 text-[10px] font-bold rounded-full ${styles[status]}`}
      >
        {status}
      </span>
    );
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Manage and review operational requests.
        </p>
      </div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-gray-700 text-sm">
              {isLoading ? (
                ...
                filteredApprovals.map((req) => (
                  ...
                  >
                    <td className="p-4 font-mono font-medium text-slate-700 dark:text-gray-200">
                      {req.id}
                    </td>
            ...
            <div className="p-6 overflow-y-auto flex-1 sidebar-scrollbar space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-800 dark:text-gray-200 mt-1">
                    {selectedRequest.relatedEntityName}
                  </p>
                </div>
                <div>
                  <p className="text-xs font-semibold text-gray-500 uppercase">
              ...
              {/* Values Diff */}
              {(selectedRequest.existingValues ||
                selectedRequest.proposedValues) && (
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="border border-red-200 bg-red-50/50 dark:border-red-900/30 dark:bg-red-900/10 rounded-lg p-4">
                      <p className="text-xs font-bold text-red-600 dark:text-red-400 mb-2 uppercase">
                        Current Value
                      </p>
                    ...
                    <div className="border border-green-200 bg-green-50/50 dark:border-green-900/30 dark:bg-green-900/10 rounded-lg p-4">
                      <p className="text-xs font-bold text-green-600 dark:text-green-400 mb-2 uppercase">
                        Proposed Value
                      </p>
                      <pre className="text-sm text-gray-700 dark:text-gray-300 font-mono whitespace-pre-wrap">
                        {JSON.stringify(
