  ...
  return (
    <div className="space-y-4 font-sans pb-4">
      <div className="sticky top-0 bg-slate-50 pt-2 pb-4 z-10">
        ...
        <p className="text-sm text-gray-500 mt-1">Manage your pending workload.</p>
        ...
        <div className="flex gap-2 mt-4">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-green-500 focus:ring-1 focus:ring-green-500"
            />
          </div>
          <button className="p-2 border border-gray-200 rounded-xl bg-white text-gray-600 active:bg-gray-50 transition-colors">
            <Filter size={20} />
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-8 text-center border border-gray-100 shadow-sm mt-8">
          ...
          <p className="text-sm text-gray-500 mt-1">You have no pending tasks assigned at the moment.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {tasks.map((task) => (
            <div key={task.id} className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm active:scale-[0.98] transition-transform flex flex-col">
              <div className="flex justify-between items-start mb-2">
                <div>
                  <h3 className="font-bold text-gray-800">{task.woNumber}</h3>
                  <p className="text-xs font-medium text-gray-500 mt-0.5">{task.product?.name || 'Unknown Product'}</p>
                </div>
              ...
              <div className="flex justify-between items-end mt-auto pt-4">
                <div className="text-xs text-gray-500">
                  <p>Target Qty: <span className="font-semibold text-gray-700">{task.requiredQty}</span></p>
                  {task.batchNumber && <p>Batch: <span className="font-semibold text-gray-700">{task.batchNumber}</span></p>}
                </div>
                <button 
