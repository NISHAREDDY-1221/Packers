...
export const QualityCheck: React.FC = () => {
  ...
  const passedToday = todaysQCs.filter((qc) =>
    ["Pass", "Partial Pass"].includes(qc.result),
  ).length;
  const failedToday = todaysQCs.filter((qc) =>
    ["Reject", "Discard"].includes(qc.result),
  ).length;
  const reworkToday = todaysQCs.filter((qc) => qc.result === "Rework").length;
  const passRate =
    todaysInspections > 0
      ? Math.round((passedToday / todaysInspections) * 100)
      : 0;
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      newFiles.forEach((fileObj) => {
        setFormPhotos((prev) =>
          prev.map((p) => (p.id === fileObj.id ? { ...p, status: "done" } : p)),
        );
      });
    }
  };
  ...
  const filteredQCs = qualityChecks.filter((qc) => {
    return (
      qc.productName.toLowerCase().includes(search.toLowerCase()) ||
      qc.woNo.toLowerCase().includes(search.toLowerCase())
    );
  });
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs space-y-4 lg:col-span-1">
          ...
          <div className="space-y-3">
            {pendingQC_WOs.map((wo, index) => {
              ...
              const batchNo =
                wo.batchNumber || `BATCH-2026-${wo.woNo.split("-").pop()}`;
              const packedQty = wo.actualProduced || wo.requiredQuantity;
              const priority = wo.priority || "High";
              const waitingSince = wo.expectedCompletion
                ? formatDateString(wo.expectedCompletion)
                : "2 hours ago";
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    <span className="text-[10px] bg-amber-100 text-amber-800 px-2 py-0.5 rounded font-bold uppercase tracking-wide">
                      Pending
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[11px]">
                    ...
                    <div>
                      ...
                      <span
                        className={`font-medium ${priority === "Urgent" || priority === "High" ? "text-rose-600" : "text-slate-700"}`}
                      >
                        {priority}
                      </span>
                    </div>
                ...
                {filteredQCs.map((qc) => (
                  <tr key={qc.id} className="hover:bg-slate-50/50">
                    <td className="px-2 py-3 font-medium whitespace-nowrap">
                      {formatDateString(qc.date)}
                    </td>
                    <td className="px-2 py-3 font-mono font-semibold text-slate-500 whitespace-nowrap">
                      {qc.id}
                    </td>
                    <td className="px-2 py-3">
                      <div
                        className="font-semibold text-slate-900 truncate max-w-[120px]"
                        title={qc.productName}
                      >
                        {qc.productName}
                      </div>
                      <div
                        className="font-mono text-slate-400 text-[10px] truncate max-w-[120px]"
                        title={qc.woNo}
                      >
                        {qc.woNo}
                      </div>
                    ...
                    <td
                      className="px-2 py-3 font-mono text-[11px] whitespace-nowrap truncate max-w-[100px]"
                      title={qc.batchNo}
                    >
                      {qc.batchNo}
                    </td>
                    <td
                      className="px-2 py-3 text-[11px] truncate max-w-[100px]"
                      title={qc.inspector}
                    >
                      {qc.inspector}
                    </td>
                    ...
                    <td className="px-2 py-3 whitespace-nowrap text-center">
                      <span
                        className={`inline-block px-2 py-0.5 rounded-full font-bold border text-[10px] ${getResultBadge(qc.result)}`}
                      >
                        {qc.result}
                      </span>
                    </td>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-xl border border-slate-200 w-full max-w-lg shadow-2xl p-6 text-left space-y-4 max-h-[90vh] overflow-y-auto sidebar-scrollbar">
            <div className="flex justify-between items-start border-b border-slate-100 pb-3 sticky top-0 bg-white z-10 pt-2 -mt-2">
              <div>
                ...
                <h3 className="font-bold text-slate-800 text-base">
                  {selectedQC.productName}
                </h3>
              </div>
              <button
                onClick={() => setSelectedQC(null)}
                className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-500 hover:text-rose-600 transition-colors cursor-pointer bg-slate-50"
              >
            ...
            <div className="grid grid-cols-2 gap-3 text-xs bg-slate-50 p-3 rounded-lg border border-slate-100">
              <div>
                <strong>WO No:</strong> {selectedQC.woNo}
              </div>
              <div>
                <strong>Batch No:</strong> {selectedQC.batchNo}
              </div>
              <div>
                <strong>Inspector:</strong> {selectedQC.inspector}
              </div>
              <div>
                <strong>Date:</strong> {formatDateString(selectedQC.date)}
              </div>
              <div>
              ...
              <div>
                ...
                <span
                  className={`px-2 py-0.5 rounded font-bold border ${getResultBadge(selectedQC.result)}`}
                >
                  {selectedQC.result}
                </span>
              </div>
              {selectedQC.severity && (
                <div>
                  ...
                  <span className="px-2 py-0.5 rounded font-bold border bg-rose-100 text-rose-800 border-rose-200">
                    {selectedQC.severity}
                  </span>
                </div>
              )}
              {selectedQC.failureReason && (
                <div>
                  <strong>Failure Reason:</strong> {selectedQC.failureReason}
                </div>
              )}
            </div>
            ...
            <div className="space-y-2">
              <h4 className="text-xs font-bold text-slate-500 uppercase">
                Inspections Checked Matrix
              </h4>
              <div className="grid grid-cols-2 gap-2 text-xs">
                {Object.entries(selectedQC.checks).map(([check, passed]) => (
                  <div
                    key={check}
                    className="flex justify-between p-2 border border-slate-100 rounded-md bg-white"
                  >
            ...
            <div className="text-xs border-t border-slate-100 pt-3">
              ...
              <p className="text-slate-600 mt-1 italic">
                {selectedQC.remarks || "No remarks added."}
              </p>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs flex justify-center items-center z-50">
          <div className="bg-white rounded-xl shadow-2xl border border-slate-200 w-full max-w-xl max-h-[90vh] overflow-y-auto flex flex-col">
            ...
            <form
              ...
              className="p-6 space-y-5 flex-1 text-left text-xs"
            >
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="Sampling Inspection">
                        Sampling Inspection
                      </option>
                        ...
                        formPackedQty > 0
                          ? (() => {
                              const pct = Math.min(
                                100,
                                Math.round(
                                  (formCheckedQty / formPackedQty) * 100,
                                ),
                              );
                              if (pct >= 50)
              ...
              <div className="border border-slate-100 rounded-lg bg-slate-50 overflow-hidden">
                ...
                {(
                  [
                    ...
                    border: string;
                    bg: string;
                    items: { key: keyof typeof checks; label: string }[];
                  }[]
                ).map(({ section, color, border, bg, items }) => (
                  <div
                    key={section}
                    className="px-3 pt-2.5 pb-3 space-y-2 border-b border-slate-100 last:border-b-0"
                  >
                    <span
                      className={`block text-[10px] font-bold uppercase tracking-wide ${color}`}
                    >
                      {section}
                    </span>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (
                        !["Rework", "Reject", "Discard"].includes(
                          e.target.value,
                        )
                      ) {
                        setFormFailureReason("");
                ...
                <div className="col-span-2 space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {formPhotos.map((photo) => (
                      <div
                        key={photo.id}
                        className="relative group w-14 h-14 rounded-lg border border-slate-200 overflow-hidden bg-slate-100 flex items-center justify-center"
                      >
                        <img
                          src={photo.url}
                          alt="upload"
                          className={`object-cover w-full h-full cursor-pointer ${photo.status === "uploading" ? "opacity-50 blur-[2px]" : ""}`}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      value={formSeverity}
                      onChange={(e) =>
                        setFormSeverity(e.target.value as IQC["severity"])
                      }
                    >
                  ...
                  <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 h-[calc(100%-24px)] flex flex-col justify-center space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="font-mono text-slate-400 italic">
                          Pending Submission...
                        </div>
                      </div>
                    </div>
      ...
      {previewPhoto && (
        ...
        >
          <div className="relative max-w-4xl max-h-full">
            <button
              className="absolute -top-4 -right-4 bg-white rounded-full p-1 text-slate-800 shadow hover:bg-slate-100 cursor-pointer"
              onClick={() => setPreviewPhoto(null)}
            >
              <X size={20} />
            </button>
