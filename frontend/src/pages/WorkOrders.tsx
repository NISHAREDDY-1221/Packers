...
import type { Recipe, User } from "../api/masterDataService";
...
const SearchableSelect = ({ options, value, onChange, placeholder }: { options: string[], value: string, onChange: (v: string) => void, placeholder: string }) => {
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      <div 
        className="w-full p-2 border border-gray-200 rounded-lg bg-white flex justify-between items-center cursor-pointer min-h-[38px]"
        onClick={() => setIsOpen(!isOpen)}
      >
        <span className={value ? "text-gray-900" : "text-gray-400"}>{value || placeholder}</span>
        <ChevronRight size={14} className={`transform transition-transform ${isOpen ? "rotate-90" : "rotate-0"} text-gray-400`} />
      </div>
        ...
        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg">
          <div className="p-2 border-b border-gray-100">
            <input
              type="text"
              autoFocus
              className="w-full p-1.5 text-xs border border-gray-300 rounded focus:outline-none focus:border-[#00891D]"
              placeholder="Search..."
              value={search}
          ...
          <div className="max-h-48 overflow-y-auto">
            {filteredOptions.length > 0 ? (
              filteredOptions.map((opt) => (
                <div
                  key={opt}
                  className={`p-2 text-sm cursor-pointer hover:bg-gray-50 ${value === opt ? 'bg-[#00891D]/10 text-[#00891D] font-medium' : 'text-gray-700'}`}
                  onClick={() => {
                    onChange(opt);
                    setIsOpen(false);
                    setSearch("");
              ...
              <div className="p-2 text-xs text-gray-400 text-center">No results found</div>
            )}
          </div>
        </div>
      )}
...
export const WorkOrders: React.FC = () => {
  ...
  const fetchRecipes = async () => {
    ...
    try {
      ...
      setApiRecipesError(
        err.response?.data?.message || "Failed to load recipes from server.",
      );
    } finally {
  ...
  React.useEffect(() => {
    masterDataService.getUsers().then(setApiUsers).catch(console.error);
  }, []);
  ...
  const [formAssignedTeam, setFormAssignedTeam] =
    useState("Packing Team Alpha");
  const [formSupervisor, setFormSupervisor] = useState("Suresh Kumar");
  const [formExpectedCompletion, setFormExpectedCompletion] =
    useState("2026-07-20");
  ...
  const categories = useMemo(
    () =>
      Array.from(
        new Set(
          workOrders.map((wo) => wo.product?.category?.name || "Unknown"),
        ),
      ),
    [workOrders],
  );
  const supervisors = useMemo(
    () => {
      const dbUsers = apiUsers
        .filter(u => !u.role || u.role.name === 'OPERATOR')
        .map(u => u.name);
      return Array.from(new Set(dbUsers));
    },
    [apiUsers]
  );
  const teams = useMemo(
    () => ["Packing Team Alpha", "Packing Team Beta", "Quality Assurance"],
    [],
  );
  ...
  const summary = useMemo(() => {
    return {
      ...
      packingStarted: workOrders.filter((w) => w.status === "PACKING_STARTED")
        .length,
  ...
  const filteredWOs = useMemo(() => {
    return workOrders.filter((wo) => {
      ...
      const matchesSupervisor =
        ...
        (wo.supervisor?.name || "") === filterSupervisor;
      let woDatePrefix = "";
      if (wo.expectedDate) {
        try {
          woDatePrefix = new Date(wo.expectedDate).toISOString().substring(0, 10);
        } catch (e) {
          woDatePrefix = wo.expectedDate.substring(0, 10);
        }
      }
  ...
  const handleCreateWO = async (e: React.FormEvent) => {
    ...
    const selectedSupervisor = apiUsers.find(u => u.name === formSupervisor);
    const supervisorIdToUse = selectedSupervisor?.id || "00000000-0000-0000-0000-000000000000";
    ...
    try {
      await workOrderService.createWorkOrder({
        ...
        expectedDate: formExpectedCompletion
          ? new Date(formExpectedCompletion).toISOString()
          : undefined,
    ...
    if (wo) {
      if (canTransition(wo.status, targetStatus)) {
        workOrderService
          ...
          .catch(console.error);
        showToast(
          `Workflow status updated: ${wo.woNumber || wo.id} is now ${targetStatus}.`,
        );
      } else {
        showToast(
          `Transition not allowed from ${wo.status} to ${targetStatus}.`,
        );
      }
    }
  };
    ...
    return [
      {
        ...
        desc: `Created on ${selectedWO.createdAt}.`,
      },
      {
        status: "PENDING",
        label: "Submitted for Approval",
        desc: `Awaiting store manager authorization.`,
      },
      {
        status: "APPROVED",
        label: "Authorized & Approved",
        desc: `Scheduled for team ${"Team Alpha"}.`,
      },
      {
        status: "MATERIAL_ISSUED",
        label: "Raw Materials Issued",
        desc: `BOM inventory dispatched to packing line.`,
      },
      {
        status: "PACKING_STARTED",
        label: "Packing Execution Started",
        desc: `Currently active on floor.`,
      },
      {
        status: "QC_PENDING",
        label: "QC Audit Verification",
        desc: `Quality checks pending evaluation.`,
      },
      {
        status: "COMPLETED",
        label: "COMPLETED",
        desc: `Finished goods safely posted.`,
      },
    ];
  }, [selectedWO]);
    ...
    <div className="space-y-5 text-left font-sans select-none pb-12">
      ...
      <div className="flex justify-between items-center border-b border-gray-200 pb-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Create, approve, assign and monitor packing work orders.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-7 gap-3">
        {[
          ...
          {
            key: "packing_started",
            title: "PACKING_STARTED",
            count: summary.packingStarted,
          },
          ...
          <div
            key={card.key}
            onClick={() =>
              setKpiFilter((prev) => (prev === card.key ? "all" : card.key))
            }
          ...
          >
            <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wide truncate block">
              {card.title}
            </span>
      ...
      <div className="bg-white p-4 border border-gray-200 rounded-lg space-y-3">
          ...
          <div className="flex border border-gray-250 rounded-lg p-0.5 bg-gray-50">
            <button
              ...
              className={`flex items-center gap-1 px-3.5 py-1 rounded-md text-xs font-semibold cursor-pointer ${
                viewMode === "table"
                  ? "bg-white shadow-xs text-[#00891D] font-bold"
                  : "text-gray-505"
              }`}
            >
              <TableIcon size={14} />
        ...
        {(search ||
          ...
          <div className="flex justify-between items-center text-[11px] text-gray-500 pt-1.5 border-t border-gray-100 font-medium">
            <span>
              Showing {filteredWOs.length} of {workOrders.length} orders
            </span>
            <button
              onClick={() => {
                setSearch("");
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-16 text-center space-y-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-gray-400">
              There are no work orders configured.
            </p>
          </div>
          <button
            onClick={() => setIsCreateOpen(true)}
          ...
          <div className="overflow-x-auto max-h-[60vh] table-scrollbar">
            <table className="w-full text-left text-xs border-collapse">
              <thead className="bg-gray-50 sticky top-0 z-10 border-b border-gray-200">
                <tr>
                  <th className="p-3 font-semibold text-gray-500 uppercase">
                    Work Order No
                  </th>
                  <th className="p-3 font-semibold text-gray-500 uppercase">
                    Product
                  ...
                  <th className="p-3 font-semibold text-gray-505 uppercase text-right">
                    Required Quantity
                  </th>

                  <th className="p-3 font-semibold text-gray-505 uppercase">
                    Assigned To
                  </th>
                  <th className="p-3 font-semibold text-gray-500 uppercase">
                    Priority
                  ...
                  <th className="p-3 font-semibold text-gray-550 uppercase">
                    Expected Completion
                  </th>
                  <th className="p-3 font-semibold text-gray-500 uppercase text-right">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {paginatedWOs.map((wo) => (
                  ...
                  >
                    ...
                    <td className="p-3">
                      <span
                        className={`px-2 py-0.5 rounded text-[10px] font-semibold border ${getPriorityStyle(wo.priority)}`}
                      >
                        {wo.priority}
                      </span>
                    </td>
                    <td className="p-3">
                      <span
                        className={`px-2 py-0.5 rounded text-[10px] font-bold border uppercase tracking-normal whitespace-nowrap ${getStatusColor(wo.status)}`}
                      >
                        {wo.status}
                      </span>
                    </td>
                    <td className="p-3 text-gray-600 font-mono">
                      {wo.expectedDate ? new Date(wo.expectedDate).toLocaleDateString() : ""}
                    </td>
                    <td
          ...
          <div className="bg-gray-50 px-4 py-3 border-t border-gray-200 flex items-center justify-between text-xs text-gray-500">
            <div className="flex items-center gap-2">
              ...
              <span>
                ...
                {Math.min(currentPage * rowsPerPage, filteredWOs.length)} of{" "}
                {filteredWOs.length} orders
              </span>
            </div>
                    ...
                    {columnWOs.map((wo) => (
                      ...
                      >
                        <div className="flex justify-between items-start">
                          ...
                          <span
                            className={`text-[9px] px-1.5 py-0.5 rounded font-semibold border ${getPriorityStyle(wo.priority)}`}
                          >
                            {wo.priority}
                          </span>
                        </div>
                        ...
                        <div className="text-xs">
                          ...
                          <span className="text-[9px] text-gray-450 block font-medium mt-0.5">
                            Recipe/BOM: {wo.recipe?.code || wo.recipeId}
                          </span>
                        </div>
                        ...
                        <div className="text-[10px] text-gray-500 space-y-0.5 pt-1 border-t border-gray-100">
                          ...
                          <div className="flex justify-between">
                            <span>Due Date:</span>
                            <span className="font-mono font-bold text-gray-650">
                              {wo.expectedDate || ""}
                            </span>
                          </div>
                        </div>
          ...
          <div className="bg-white w-full max-w-md h-screen overflow-y-auto p-6 shadow-2xl flex flex-col justify-between">
            <div className="space-y-6 text-left">
              <div className="flex justify-between items-center border-b border-gray-200 pb-3">
                <div>
                  <span className="text-[9px] font-mono font-bold text-[#00891D] uppercase tracking-widest">
                    {selectedWO.woNumber}
                  </span>
              ...
              <div className="border border-gray-200 rounded-lg divide-y divide-gray-150 overflow-hidden bg-white text-xs">
                ...
                <div className="flex justify-between p-2.5">
                  <span className="text-gray-505 font-medium">
                    Recipe / BOM
                  </span>
                ...
                <div className="flex justify-between p-2.5">
                  ...
                  <span className="font-bold text-gray-900">
                    {selectedWO.requiredQty} Units
                  </span>
                </div>

                <div className="flex justify-between p-2.5">
                  <span className="text-gray-505 font-medium">Assigned To</span>
                  <span className="font-semibold text-gray-900">
                    {selectedWO.supervisor?.name || "Unassigned"}
                  </span>
                </div>
                <div className="flex justify-between p-2.5">
                  <span className="text-gray-505 font-medium">Priority</span>
                  <span
                    className={`px-2 py-0.5 rounded text-[10px] font-bold border ${getPriorityStyle(selectedWO.priority)}`}
                  >
                    {selectedWO.priority}
                  </span>
                </div>
                <div className="flex justify-between p-2.5">
                  ...
                  <span
                    className={`px-2 py-0.5 rounded text-[10px] font-bold border uppercase tracking-wider ${getStatusColor(selectedWO.status)}`}
                  >
                    {selectedWO.status}
                  </span>
                </div>
              ...
              <div className="space-y-3 pt-2">
                ...
                <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg space-y-4 relative before:absolute before:left-5 before:top-4 before:bottom-4 before:w-0.5 before:bg-gray-200">
                  {timelineSteps.map((step, idx) => {
                    ...
                    const isPast =
                      KANBAN_COLUMNS.indexOf(selectedWO.status) >=
                      KANBAN_COLUMNS.indexOf(step.status);

                    return (
                      <div
                        key={idx}
                        className="flex gap-3 relative z-10 text-xs"
                      >
                        <div
                          className={`w-4 h-4 rounded-full flex items-center justify-center text-white border-2 border-white ${
                            isActive
                              ? "bg-[#00891D] ring-2 ring-[#00891D]/10"
                              : isPast
                                ? "bg-[#00891D]"
                                : "bg-gray-300"
                          }`}
                        ...
                        <div>
                          <span
                            className={`font-bold block ${isActive ? "text-[#00891D]" : "text-gray-700"}`}
                          >
                            {step.label}
                          </span>
                      ...
                      onClick={() => {
                        if (confirm(`Delete draft ${selectedWO.woNumber}?`)) {
                          workOrderService
                            ...
                            .then(() => {
                              showToast("Cancelled work order.");
                              setSelectedWO(null);
                              fetchWorkOrders();
                            })
                            .catch(console.error);
          ...
          <div className="bg-white rounded-xl shadow-2xl border border-gray-200 w-full max-w-md max-h-[90vh] overflow-y-auto flex flex-col">
            <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-gray-900 text-white rounded-t-xl">
              ...
              <button
                ...
                className="p-1 rounded hover:bg-gray-850 text-gray-400 hover:text-white cursor-pointer"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <form
              ...
              className="p-5 space-y-4 flex-1 text-left text-xs"
            >
              <div>
                <label className="block text-[10px] font-bold text-gray-500 uppercase mb-1 flex justify-between items-center">
                  ...
                  <button
                    ...
                    className="text-[#00891D] hover:underline"
                    disabled={apiRecipesLoading}
                  >
                ...
                {apiRecipesError ? (
                  ...
                  <div className="text-gray-500 text-xs py-2">
                    No eligible recipes available. Create one in Master Data
                    first.
                  </div>
                ) : (
                  <select
                  ...
                  >
                    <option value="">
                      {apiRecipesLoading
                        ? "Loading recipes..."
                        : "-- Choose Recipe Configuration --"}
                    </option>
                    {apiRecipes
                      .filter((r) => r.isActive !== false)
                      .map((r) => (
                        <option key={r.id} value={r.id}>
                          {r.code} - {r.name} - {r.outputProduct?.name}
                        </option>
                      ))}
                  </select>
                )}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    value={formPriority}
                    onChange={(e) =>
                      setFormPriority(e.target.value as WorkOrder["priority"])
                    }
                  >
                    <option value="LOW">Low</option>
                    <option value="MEDIUM">Medium</option>
              ...
              {selectedRecipe && (
                <div className="bg-gray-50 p-3 rounded-lg border border-gray-150 space-y-1 text-[11px]">
                  <span className="text-[9px] font-bold text-gray-400 uppercase tracking-wider block mb-1">
                    BOM Raw Materials & Packaging
                  </span>
                  {selectedRecipe.items?.map((item, idx) => {
                    const reqQty =
                      (item.requiredQty / selectedRecipe.outputQty) * formQty;
                    return (
                      <div
                        key={idx}
                        className="flex justify-between text-gray-650 font-medium"
                      >
                        <span>{item.inputProduct?.name}</span>
                        <span className="font-bold text-gray-800">
                          {reqQty.toFixed(1)}{" "}
                          {item.isPackaging ? "Units" : "kg"}
                        </span>
                      </div>
                    );
                  })}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    value={editPriority}
                    onChange={(e) =>
                      setEditPriority(e.target.value as WorkOrder["priority"])
                    }
                  >
                    <option value="LOW">Low</option>
                    <option value="MEDIUM">Medium</option>
