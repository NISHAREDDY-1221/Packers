...
export class ReportService {
  static async getProductionYield(startDate: Date, endDate: Date) {
    ...
    Object.values(yieldReport).forEach((item: any) => {
      item.yieldPercentage = item.totalRequired > 0 
        ? ((item.totalProduced / item.totalRequired) * 100).toFixed(2) + '%' 
        : '0%';
      item.rejectRate = (item.totalProduced + item.totalRejected) > 0
        ? ((item.totalRejected / (item.totalProduced + item.totalRejected)) * 100).toFixed(2) + '%'
        : '0%';
    });
