...
export const createWorkOrder = catchAsync(
  async (req: Request, res: Response) => {
    const { productId, recipeId, requiredQty, priority, expectedDate } =
      req.body;
...
export const updateWorkOrderStatus = catchAsync(
  async (req: Request, res: Response) => {
    const id = req.params.id as string;
    const { status } = req.body;
    const userId = req.user!.id;

    const updatedWO = await WorkOrderService.updateWorkOrderStatus(
      id,
      status,
      userId,
    );
...
export const issueMaterials = catchAsync(
  async (req: Request, res: Response) => {
    const id = req.params.id as string;
...
export const completePacking = catchAsync(
  async (req: Request, res: Response) => {
    const id = req.params.id as string;
    const { actualProduced, actualRejected } = req.body;
    const userId = req.user!.id;

    const updatedWO = await WorkOrderService.completePacking(
      id,
      actualProduced,
      actualRejected,
      userId,
